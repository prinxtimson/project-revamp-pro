...
const CallbackChartWidget = ({ handleOnRemoveWidget }) => {
    const menuRef = useRef();
    const settingMenuRef = useRef();
    const [chartData, setChartData] = useState({});
    const [chartOptions, setChartOptions] = useState({});

    const { summary } = useSelector((state) => state.callback);
            ...
            <div className="">
                <div className="tw-float-right tw-flex tw-items-center tw-gap-2">
                    ...
                    <>
                        ...
                        <Menu model={CALLBACKS} ref={menuRef} popup />
                    </>
                </div>
...
export default CallbackChartWidget;
...
const CALLBACKS = [
    {
        ...
        value: "xlsx",
        command: () => {
            window.open(`/callback/report/download?type=xlsx`, "_blank");
        },
    },
    {
        label: "CSV",
        value: "csv",
        command: () => {
            window.open(`/callback/report/download?type=csv`, "_blank");
        },
    },
    {
        label: "PDF",
        value: "pdf",
        command: () => {
            window.open(`/callback/report/download?type=pdf`, "_blank");
        },
    },
];
